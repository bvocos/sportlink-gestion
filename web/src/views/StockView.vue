...
const filters = ref({ depositoId: '', desde: '', hasta: '' })
const form = ref({ depositoId: '', cantidadM2: 0, observaciones: '' })
...
function openIngreso() {
  form.value = { depositoId: depositosIngreso.value[0]?.id ?? '', cantidadM2: 0, observaciones: '' }
  saveError.value = ''
...
async function clearFilters() {
  filters.value = { depositoId: '', desde: '', hasta: '' }
  await applyFilters()
}
  ...
  <section class="page">
    ...
    <template v-else>
      <div class="grid stock-cards">
        <article v-for="deposito in depositos" :key="deposito.id" class="card metric">
          <small>{{ deposito.nombre }}</small>
          <strong :class="{ negative: deposito.stockActualM2 < 0 }">{{ meters(deposito.stockActualM2) }}</strong>
          <em>Stock actual</em>
        </article>
      </div>
      ...
      <div class="panel">
        ...
        <table v-else>
          <thead><tr><th>Fecha</th><th>Depósito</th><th>Tipo</th><th>Usuario</th><th>Observaciones</th><th class="num">Cantidad</th></tr></thead>
          <tbody><tr v-for="movimiento in movimientos" :key="movimiento.id"><td>{{ new Date(movimiento.fecha).toLocaleString('es-AR') }}</td><td><b>{{ movimiento.depositoNombre }}</b></td><td><span class="badge" :class="{ warn: movimiento.tipo === 'Ajuste', danger: movimiento.tipo === 'SalidaPorVenta' }">{{ movimiento.tipo }}</span></td><td>{{ movimiento.usuario }}</td><td>{{ movimiento.observaciones || '—' }}</td><td class="num"><b :class="{ negative: movimiento.cantidadConSigno < 0 }">{{ movimiento.cantidadConSigno > 0 ? '+ ' : '− ' }}{{ meters(Math.abs(movimiento.cantidadConSigno)) }}</b></td></tr></tbody>
        </table>
