...
async function load(reset = false) {
  ...
  try {
    const { data } = await http.get("/auditoria", {
      params: { ...filters.value, page: page.value, pageSize },
    });
  ...
  <section class="page">
    <div class="page-title">
      <div>
        ...
        <p>Historial de las operaciones realizadas por los usuarios.</p>
      </div>
    </div>
    <form class="audit-filters panel" @submit.prevent="load(true)">
      <div class="field">
      ...
      <div class="field">
        <label>Desde</label><input v-model="filters.desde" type="date" />
      </div>
      <div class="field">
        <label>Hasta</label><input v-model="filters.hasta" type="date" />
      </div>
      <div class="field audit-search">
        <label>Usuario, venta o movimiento</label
        ><input
          v-model="filters.buscar"
          placeholder="Buscar usuario, ID o cambio"
        />
      </div>
      <div class="filter-actions"><button class="btn">Filtrar</button></div>
    </form>
        ...
        <tbody>
          <tr v-for="x in items" :key="x.id">
            ...
            <td>
              <span
                class="badge"
                :class="{ warn: x.accion === 'Eliminación' }"
                >{{ x.accion }}</span
              >
            </td>
            <td>
              {{ x.entidad }}<small class="entity-id">{{ x.entidadId }}</small>
            </td>
            <td>
              <button
                class="icon-btn"
    ...
    <div v-if="selected" class="modal-bg" @click.self="selected = null">
      <div class="modal">
        ...
        <p>
          <b>{{ selected.usuario }}</b> · {{ dateTime(selected.fechaHora) }} ·
          {{ selected.entidad }}
        </p>
        <table class="changes">
          <thead>
          ...
          <tbody>
            <tr v-for="[field, change] in details(selected)" :key="field">
              <td>
                <b>{{ fieldLabel(field) }}</b>
              </td>
              <td>{{ value(change.anterior) }}</td>
              <td>{{ value(change.nuevo) }}</td>
            </tr>
          </tbody>
        </table>
        <div class="actions">
