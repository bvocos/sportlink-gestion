...
public static class CuotaEndpoints
{
 ...
 public static void MapCuotaEndpoints(this IEndpointRouteBuilder app){var g=app.MapGroup("/api/cuotas").WithTags("Cuotas").RequireAuthorization("cuotas");g.MapGet("/pendientes",(AppDbContext db,CancellationToken ct)=>List(db,false,ct));g.MapGet("/abonadas",(AppDbContext db,CancellationToken ct)=>List(db,true,ct));g.MapPut("/{id:guid}/vencimiento",ActualizarVencimiento);g.MapPost("/{id:guid}/pagos",RegistrarPago);g.MapPost("/{id:guid}/anular-pago",AnularPago);}
