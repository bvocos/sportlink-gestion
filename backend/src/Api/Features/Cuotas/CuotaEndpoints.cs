...
public static class CuotaEndpoints
{
 ...
 public static void MapCuotaEndpoints(this IEndpointRouteBuilder app){var g=app.MapGroup("/api/cuotas").WithTags("Cuotas").RequireAuthorization("cuotas");g.MapGet("/pendientes",(AppDbContext db,CancellationToken ct)=>List(db,false,ct));g.MapGet("/pendientes/resumen",ResumenPendiente);g.MapGet("/abonadas",(AppDbContext db,CancellationToken ct)=>List(db,true,ct));g.MapPut("/{id:guid}/vencimiento",ActualizarVencimiento);g.MapPost("/{id:guid}/pagos",RegistrarPago);g.MapPost("/{id:guid}/anular-pago",AnularPago);}
 ...
 static async Task<IResult> ResumenPendiente(string? buscar,AppDbContext db,CancellationToken ct)
 {
  var q=db.Cuotas.AsNoTracking().Where(x=>x.ImportePagado<x.ImportePactado);
  var term=buscar?.Trim();
  if(!string.IsNullOrWhiteSpace(term))q=q.Where(x=>EF.Functions.Like(x.Venta.Cliente.Nombre+" "+x.Venta.Cliente.Apellido,$"%{term}%"));
  var resumen=await q.GroupBy(_=>1).Select(g=>new{cantidad=g.Count(),totalPendiente=g.Sum(x=>x.ImportePactado-x.ImportePagado)}).SingleOrDefaultAsync(ct);
 ...
 static async Task<IResult> ActualizarVencimiento(Guid id,ActualizarVencimientoRequest request,AppDbContext db,CancellationToken ct)
 {
  ...
  var cuota=await db.Cuotas.FindAsync([id],ct);
